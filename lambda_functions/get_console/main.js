...
exports.main = async (event, context, callback) => {
	...
	try {
        ...
        const session_duration = Math.floor((expires - new Date()) / 1000);

        console.log(session_duration);

		const token = await axios.get(`https://signin.aws.amazon.com/federation?Action=getSigninToken&SessionDuration=${session_duration}&Session=${encodeURIComponent(sessionString)}`);
